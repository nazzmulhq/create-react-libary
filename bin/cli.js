...
async function main() {
  ...
  libraryName = libraryName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');

  // Get author name
  const authorName = await question(`${colors.cyan}?${colors.reset} Author name (Your Name): `) || 'Your Name';
